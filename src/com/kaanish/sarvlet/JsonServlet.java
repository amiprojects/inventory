		...
		try {
			...
			case "updateSalesEntry":
				...
				try {
					...
					generatorSE.writeStartObject().write("error", true).write("msg", "Eror").writeEnd().close();
					e.printStackTrace();
				}
				break;
			...
			case "validity":
				...
				if (req.getParameter("uniqueNm").equals(ejb.getAllUniqueName().get(0).getUniqueNm())) {
					if (f == 0) {
						...
						gena.writeStartObject().write("msg", "validity successfully extended.").writeEnd().close();
					} else {
						JsonGeneratorFactory fact = Json.createGeneratorFactory(null);
						JsonGenerator gena = fact.createGenerator(resp.getOutputStream());
						gena.writeStartObject().write("msg", "you have already applied this patch.").writeEnd().close();
					}
				} else {
					JsonGeneratorFactory fact = Json.createGeneratorFactory(null);
					JsonGenerator gena = fact.createGenerator(resp.getOutputStream());
					gena.writeStartObject().write("msg", "Wrong unique name. Please try again!.").writeEnd().close();
				}
				break;
