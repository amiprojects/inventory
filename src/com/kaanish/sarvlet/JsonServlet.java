	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			case "getCurrentCreditNoteByVendorId":
				...
				if (voucherDet != null) {
					generatorVDN.writeStartObject().write("currentCreditNote", voucherDet.getTotalCreditNote())
							.writeEnd();
				} else {
			...
			case "updateSalesEntry":
				...
				try {
					...
					// voucherDetails
					// .setVoucherAssign(ejb.getVoucherAssignByCustomerId(Integer.parseInt(req
					...
					PaymentDetails paymentDetails = ejb.getPaymentDetailsBySalesEntryId(salesEntry.getId())
							.get(paySize - 1);
					if (paymentDetails.getPaymentStatus().getStatus().equals("Full Paid")) {
						// if (req.getParameter("pstatus").equals("Full Paid"))
						// {
						...
						paymentDetails.setTotalAmount(Float.parseFloat(req.getParameter("gt")));
						VoucherDetails vd = ejb.getAllVoucherDetailsBySalesEntryId(salesEntry.getId()).get(0);
					...
					for (SalesReturn sr : ejb.getSalesReturnBySalesEntryId(salesEntry.getId())) {
						...
						VoucherDetails vd = ejb.getAllVoucherDetailsBySalesReturnId(sr.getId()).get(0);
			...
			case "updatePurchaseEntry":
				...
				try {
					...
					for (PurchaseReturn pr : purchase_Entry.getPurchaseReturn()) {
						...
						payDetails.setPaidAmount(grandtotal);
						VoucherDetails vd = ejb.getAllVoucherDetailsByPurchaseReturnId(pr.getId()).get(0);
						vd.setValue(grandtotal);
						ejb.updateVoucherDetails(vd);
