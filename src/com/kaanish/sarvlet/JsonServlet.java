	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			case "getCurrentDebitNoteByCustomerId":
				...
				voucherDetails = ejb.getLastVoucherDetailsByCustomerId(Integer.parseInt(req.getParameter("id")));
				generatorCDN.writeStartObject().write("vdId", voucherDetails.getId())
						.write("currentDebitNote",
								ejb.getCurrentDebitNoteByCustomerId(Integer.parseInt(req.getParameter("id"))))
						.writeEnd();
				generatorCDN.close();
				break;
			...
			case "getCurrentCreditNoteByVendorId":
				...
				generatorVDN.writeStartObject()
						.write("currentCreditNote",
								ejb.getCurrentCreditNoteByVendorId(Integer.parseInt(req.getParameter("id"))))
						.writeEnd();
				generatorVDN.close();
				break;
			...
			case "updateSalesEntry":
				...
				try {
					...
					voucherDetails.setValue(Float.parseFloat(req.getParameter("gt")) - paymentDetails.getPaidAmount());
					ejb.updateVoucherDetails(voucherDetails);
					...
					if (salesEntry.getVendor() != null) {
						...
						if (ejb.getAllVoucherDetails4ViaAgentBySalesEntryId(salesEntry.getId()).size() > 0) {
							...
							for (VoucherDetailsForViaAgents voucherDetForViaAgent : ejb
									.getAllVoucherDetails4ViaAgentBySalesEntryId(salesEntry.getId())) {
						...
						if (ejb.getPaymentDetails4ViaAgentBySalesEntryId(salesEntry.getId()).size() > 0) {
							for (PaymentDetailsForViaAgents payDetailsForViaAgents : ejb
									.getPaymentDetails4ViaAgentBySalesEntryId(salesEntry.getId())) {
								ejb.deletePaymentDetails4ViaAgent(payDetailsForViaAgents.getId());
							}
						}
					}
					...
					for (SalesReturn sr : ejb.getSalesReturnBySalesEntryId(salesEntry.getId())) {
						...
						if (salesEntry.getVendor() != null && salesEntry.isEfectiveProfit()) {
							if (ejb.getAllVoucherDetails4ViaAgentBySalesReturnId(sr.getId()).size() > 0) {
								...
								for (VoucherDetailsForViaAgents voucherDetForViaAgent : ejb
										.getAllVoucherDetails4ViaAgentBySalesReturnId(sr.getId())) {
									voucherDetForViaAgent.setAgentId(salesEntry.getVendor().getId());
									ejb.updateVoucherDetails4ViaAgent(voucherDetForViaAgent);
								}
							} else {
