	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			case "addBillSetup":
				...
				billSetup.setCompanyInfo(ejb.getUserById((String) httpSession.getAttribute("user")).getCompanyInfo());
			...
			case "updateVendor":
				...
				if (!req.getParameter("vendorServiceTaxRegDate").equals("")) {
					accountDetails
							.setCstRegistrationDate(DateConverter.getDate(req.getParameter("vendorServiceTaxRegDate")));
				}
			...
			case "purchaseReturn":
				...
				if (chPR == 0) {
					...
					paymentDetails.setPurchase_Entry(purchaseEntry);
					paymentDetails.setPaymentType(ejb.getPaymentTypeByType(req.getParameter("pType")));
					if (ejb.getPaymentDetailsByPurchaseEntryId(purchaseEntry.getId()).get(0).getTotalAmount() - ejb
							.getPaymentDetailsByPurchaseEntryId(purchaseEntry.getId()).get(0).getPaidAmount() > Float
									.parseFloat(req.getParameter("spAmount"))) {
			...
			case "purchaseSearchByDate":
				...
				if (purEntryList.size() > 0) {
					...
					msg = "No result found for dated " + req.getParameter("fDate") + " to " + req.getParameter("lDate")
							+ "...";
				}
				break;
			...
			case "purchasePayment":
				...
				if (ejb.getAllVoucherDetailsByPurchaseEntryId(Integer.parseInt(req.getParameter("peId"))).size() == 0) {
					if (!req.getParameter("pstatus").equals("Full Paid")) {
						voucherDetails = new VoucherDetails();
						voucherDetails.setVoucherAssign(voucherAssign);
						voucherDetails.setCredit(true);
				...
				paymentDetails.setPaidAmount(Float.parseFloat(req.getParameter("spPaymentAmount")));
				paymentDetails.setDescription(req.getParameter("desc"));
				paymentDetails.setPurchase_Entry(ejb.getPurchaseEntryById(Integer.parseInt(req.getParameter("peId"))));
				paymentDetails.setPaymentType(ejb.getPaymentTypeByType(req.getParameter("pType")));
				paymentDetails.setPaymentStatus(ejb.getPaymentStatusByStatus(req.getParameter("pstatus")));
				ejb.setPaymentDetails(paymentDetails);
				// correcting purchase entry payment details
				for (Purchase_Entry pe : ejb.getAllPurchaseEntry()) {
					...
					if (pSize > 0) {
						...
						for (int ind = ejb.getPaymentDetailsByPurchaseEntryId(pe.getId()).size() - 1; ind > -1; ind--) {
							PaymentDetails paymentDetails = ejb.getPaymentDetailsByPurchaseEntryId(pe.getId()).get(ind);
							paymentDetails.setTotalAmount(tot);
							tot = tot - paymentDetails.getPaidAmount();
							ejb.updatePaymentDetails(paymentDetails);
						}
			...
			case "salesPayment":
				...
				if (ejb.getAllVoucherDetailsBySalesEntryId(Integer.parseInt(req.getParameter("seId"))).size() == 0) {
					if (!req.getParameter("pstatus").equals("Full Paid")) {
						voucherDetails = new VoucherDetails();
				...
				for (SalesEntry se : ejb.getAllSalesEntries()) {
					...
					if (pSize > 0) {
						float tot = ejb.getPaymentDetailsBySalesEntryId(se.getId()).get(pSize - 1).getTotalAmount();
						for (int ind = ejb.getPaymentDetailsBySalesEntryId(se.getId()).size() - 1; ind > -1; ind--) {
							PaymentDetails paymentDetails = ejb.getPaymentDetailsBySalesEntryId(se.getId()).get(ind);
							paymentDetails.setTotalAmount(tot);
							tot = tot - paymentDetails.getPaidAmount();
							ejb.updatePaymentDetails(paymentDetails);
						}
			...
			case "jobSearchByJobChallanNo":
				...
				List<JobAssignmentDetails> jobAssignList1 = ejb
						.getJobAssignByChallanNo(req.getParameter("companyInitial") + "/" + req.getParameter("fynYear")
								+ "/" + req.getParameter("month") + "/" + req.getParameter("billType") + "/"
								+ req.getParameter("autoNum") + "/" + req.getParameter("suffix"));
			...
			case "jobSearchByJobberName":
				...
				List<JobAssignmentDetails> jobAssignList2 = ejb.getJobAssignByJoberName(req.getParameter("jobberName"));
			...
			case "jobSearchByProductCode":
				page = "jobAssignSearch.jsp";
				List<JobAssignmentDetails> jobAssignList3 = ejb.getJobAssignByProdCode(req.getParameter("prodCode"));
			...
			case "jobSearchByProductCodeForPayment":
				page = "jobPayment.jsp";
				List<JobAssignmentDetails> jobAssignListPP = ejb.getJobAssignByProdCode(req.getParameter("prodCode"));
			...
			case "salesSearchBySalesChallanNo":
				...
				List<SalesEntry> salesEntryLst1 = ejb
						.getSalesEntryByChallanNo(req.getParameter("companyInitial") + "/" + req.getParameter("fynYear")
								+ "/" + req.getParameter("month") + "/" + req.getParameter("billType") + "/"
								+ req.getParameter("autoNum") + "/" + req.getParameter("suffix"));
					...
					for (int l = 0; l < p3.length; l++) {
						if (!qty4[l].equals("0")) {
							if (!qty4[l].equals("")) {
								...
								salesProductDetails
										.setSalesReQty(salesProductDetails.getSalesReQty() + Float.parseFloat(qty4[l]));
			...
			case "jobAssignmentForParticularDesignNumber":
				...
				if (chJ == 0) {
					...
					for (int l1 = 0; l1 < productForSampleId1.length; l1++) {
						...
						String[] jobForSampleIdH = req.getParameterValues("jobIdH" + productForSampleId1[l1]);
				...
				if (chJR == 0) {
					...
					for (int l = 0; l < jobPlanProductsId.length; l++) {
						...
						jobRecieveProductsDetails
								.setJobPlanProducts(ejb.getJobPlanProductsById(Integer.parseInt(jobPlanProductsId[l])));
						...
						jobPlanProducts
								.setRemainingQty(jobPlanProducts.getRemainingQty() + Float.parseFloat(prodQtyRe[l]));
						...
						jobAssignmentProducts
								.setRemaninQty(jobAssignmentProducts.getRemaninQty() - Float.parseFloat(prodQtyRe[l]));
						ejb.updateJobAssignmentProductDetails(jobAssignmentProducts);
