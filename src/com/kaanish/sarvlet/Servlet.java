...
@WebServlet({ "/logout", "/addTax", "/addTaxGroup", "/editTax", "/deleteTax", "/editTaxGroup", "/deleteTaxGroup",
		...
		"/salesReturnSearchByProductCode" })
				...
				try {
					...
					case "purchaseEntry":
						...
						if (chP1 == 0) {
							...
							for (int l = 0; l < qty.length; l++) {
								...
								purchaseProductDetails.setPurchase_Entry(purchaseEntry);
								purchaseProductDetails.setLotNumber(lot[l].toUpperCase());
					...
					case "salesEntry":
						...
						if (chS == 0) {
							...
							for (SalesEntry se : ejb.getAllSalesEntries()) {
								...
								if (pSize > 0) {
									float tot = ejb.getPaymentDetailsBySalesEntryId(se.getId()).get(pSize - 1)
											.getTotalAmount();
									for (int ind = ejb.getPaymentDetailsBySalesEntryId(se.getId()).size()
											- 1; ind > -1; ind--) {
										PaymentDetails paymentDetails = ejb.getPaymentDetailsBySalesEntryId(se.getId())
												.get(ind);
										paymentDetails.setTotalAmount(tot);
										tot = tot - paymentDetails.getPaidAmount();
										ejb.updatePaymentDetails(paymentDetails);
					...
					case "purchasePayment":
						...
						for (Purchase_Entry pe : ejb.getAllPurchaseEntry()) {
							...
							if (pSize > 0) {
								...
								for (int ind = ejb.getPaymentDetailsByPurchaseEntryId(pe.getId()).size()
										- 1; ind > -1; ind--) {
									PaymentDetails paymentDetails = ejb.getPaymentDetailsByPurchaseEntryId(pe.getId())
											.get(ind);
									paymentDetails.setTotalAmount(tot);
									tot = tot - paymentDetails.getPaidAmount();
									ejb.updatePaymentDetails(paymentDetails);
					...
					case "salesPayment":
						...
						if (ejb.getVoucherAssignByCustomerId(Integer.parseInt(req.getParameter("cId"))) == null) {
							...
							voucherAssign = ejb.getVoucherAssignByCustomerId(Integer.parseInt(req.getParameter("cId")));
						}
						...
						paymentDetails.setPaidAmount(Float.parseFloat(req.getParameter("spPaymentAmount")));
						paymentDetails.setDescription(req.getParameter("desc"));
						paymentDetails.setSalesEntry(ejb.getSalesEntryById(Integer.parseInt(req.getParameter("seId"))));
						paymentDetails.setPaymentType(ejb.getPaymentTypeByType(req.getParameter("pType")));
						paymentDetails.setPaymentStatus(ejb.getPaymentStatusByStatus(req.getParameter("pstatus")));
						ejb.setPaymentDetails(paymentDetails);
						// correcting sales entry payment details
					...
					case "jobPayment":
						...
						for (JobAssignmentDetails ja : ejb.getAllJobassignmentDetails()) {
							...
							if (pSize > 0) {
								float tot = ejb.getPaymentDetailsByJobAsignId(ja.getId()).get(pSize - 1)
										.getTotalAmount();
								for (int ind = ejb.getPaymentDetailsByJobAsignId(ja.getId()).size()
										- 1; ind > -1; ind--) {
									PaymentDetails paymentDetails = ejb.getPaymentDetailsByJobAsignId(ja.getId())
											.get(ind);
									paymentDetails.setTotalAmount(tot);
									tot = tot - paymentDetails.getPaidAmount();
									ejb.updatePaymentDetails(paymentDetails);
