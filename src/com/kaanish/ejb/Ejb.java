...
public class Ejb {
	...
	public List<VoucherDetails> getVoucherDetailsByVendorId(int id) {
		TypedQuery<VoucherDetails> q = em.createQuery(
				"select c from VoucherDetails c where c.voucherAssign.vendor.id=:id", VoucherDetails.class);
		q.setParameter("id", id);
		return q.getResultList();
	}
	...
	public List<JobPlan> getAllOngoingJobPlanByDesignNumber(int dn) {
		TypedQuery<JobPlan> q = em.createQuery(
				"select c from JobPlan c where c.designCostSheet.id =:dn and c.isComplete=false", JobPlan.class);
		q.setParameter("dn", dn);
		return q.getResultList();
	}

	public List<JobPlan> getAllJobPlanByDesignNumber(int dn) {
		TypedQuery<JobPlan> q = em.createQuery("select c from JobPlan c where c.designCostSheet.id =:dn",
				JobPlan.class);
		q.setParameter("dn", dn);
		return q.getResultList();
	}
